...
$testbench = Join-Path $repoRoot 'sim\tb_cpu_sync_misaligned.sv'
$buildDir = Join-Path $repoRoot 'build\xsim_cpu_sync_misaligned'
...
try {
    ...
    & $xelab tb_cpu_sync_misaligned -s tb_cpu_sync_misaligned_sim -debug typical --timescale 1ns/1ps -mt 8
    if ($LASTEXITCODE -ne 0) { throw "xelab failed with exit code $LASTEXITCODE" }
    $simOutput = & $xsim tb_cpu_sync_misaligned_sim -runall 2>&1
    $simExit = $LASTEXITCODE
    $simOutput | Write-Host
    if ($simExit -ne 0) { throw "xsim failed with exit code $simExit" }
    if (($simOutput | Out-String) -notmatch 'SYNC MISALIGNED PASS:') {
        throw 'Synchronous misaligned-access simulation did not report a pass result'
    }
}
finally {
    Pop-Location
