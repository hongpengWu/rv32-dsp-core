...
$testbench = Join-Path $repoRoot 'sim\tb_pl_controlled.sv'
$buildDir = Join-Path $repoRoot 'build\xsim_pl_controlled'
...
try {
    ...
    & $xelab tb_pl_controlled -s tb_pl_controlled_sim -debug typical --timescale 1ns/1ps -mt 8
    if ($LASTEXITCODE -ne 0) { throw "xelab failed with exit code $LASTEXITCODE" }
    $simOutput = & $xsim tb_pl_controlled_sim -runall 2>&1
    $simExit = $LASTEXITCODE
    $simOutput | Write-Host
    if ($simExit -ne 0) { throw "xsim failed with exit code $simExit" }
    if (($simOutput | Out-String) -notmatch 'CONTROLLED PL PASS:') {
        throw 'Controlled PL simulation did not report a pass result'
    }
}
finally {
    Pop-Location
