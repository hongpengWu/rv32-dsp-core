...
$testbench = Join-Path $repoRoot 'sim\tb_pynq_demo.sv'
$buildDir = Join-Path $repoRoot 'build\xsim_pynq_demo'
...
try {
    ...
    & $xelab tb_pynq_demo -s tb_pynq_demo_sim -debug typical --timescale 1ns/1ps -mt 8
    if ($LASTEXITCODE -ne 0) { throw "xelab failed with exit code $LASTEXITCODE" }
    $simOutput = & $xsim tb_pynq_demo_sim -runall 2>&1
    $simExit = $LASTEXITCODE
    $simOutput | Write-Host
    if ($simExit -ne 0) { throw "xsim failed with exit code $simExit" }
    if (($simOutput | Out-String) -notmatch 'PYNQ DEMO PASS:') {
        throw 'PYNQ demo simulation did not report a pass result'
    }
}
finally {
    Pop-Location
