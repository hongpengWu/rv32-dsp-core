...
$testbench = Join-Path $repoRoot 'sim\tb_pynq_sync_demo.sv'
$buildDir = Join-Path $repoRoot 'build\xsim_pynq_sync_demo'
...
try {
    ...
    & $xelab tb_pynq_sync_demo -s tb_pynq_sync_demo_sim -debug typical --timescale 1ns/1ps -mt 8
    if ($LASTEXITCODE -ne 0) { throw "xelab failed with exit code $LASTEXITCODE" }
    $simOutput = & $xsim tb_pynq_sync_demo_sim -runall 2>&1
    $simExit = $LASTEXITCODE
    $simOutput | Write-Host
    if ($simExit -ne 0) { throw "xsim failed with exit code $simExit" }
    if (($simOutput | Out-String) -notmatch 'SYNC PYNQ PASS:') {
        throw 'Synchronous PYNQ demo did not report a pass result'
    }
}
finally {
    Pop-Location
