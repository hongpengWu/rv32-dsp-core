...
$testbench = Join-Path $repoRoot 'sim\tb_cpu_sync.sv'
$buildDir = Join-Path $repoRoot 'build\xsim_cpu_sync'
...
try {
    ...
    & $xelab tb_cpu_sync -s tb_cpu_sync_sim -debug typical --timescale 1ns/1ps -mt 8
    if ($LASTEXITCODE -ne 0) { throw "xelab failed with exit code $LASTEXITCODE" }
    $simOutput = & $xsim tb_cpu_sync_sim -runall 2>&1
    $simExit = $LASTEXITCODE
    $simOutput | Write-Host
    if ($simExit -ne 0) { throw "xsim failed with exit code $simExit" }
    if (($simOutput | Out-String) -notmatch 'SYNC CORE PASS:') {
        throw 'Synchronous Core simulation did not report a pass result'
    }
}
finally {
    Pop-Location
